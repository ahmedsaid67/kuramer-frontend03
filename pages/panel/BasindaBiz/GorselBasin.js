...
export default function GorselBasin() {
    ...
    useEffect(() => {
        setIsLoading(true);
        setHasError(false);
        axios.get(`http://127.0.0.1:8000/api/appname/gorselbasin/?page=${currentPage}`)
          .then(response => {
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        axios.put(`http://127.0.0.1:8000/api/appname/gorselbasin/${editedItem.id}/`, formData)
          .then(response => {
      ...
      const handleAddNewItem = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/gorselbasin/', formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(`http://127.0.0.1:8000/api/appname/gorselbasin/?page=${currentPage}`);
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleDeleteSelected = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/gorselbasin/bulk_soft_delete/', { ids: selectedIds })
          .then(() => axios.get(`http://127.0.0.1:8000/api/appname/gorselbasin/`))
          .then(response => {
            ...
            if (updatedPage !== currentPage) {
              return axios.get(`http://127.0.0.1:8000/api/appname/gorselbasin/?page=${updatedPage}`);
            } else {
              return response;
            }
          })
          ...
          <Table>
            <TableHead>
              <StyledTableRow>
                ...
                <StyledTableCell>Yayın Linki</StyledTableCell>
            ...
            <TableBody>
              {data.map(row => (
                <StyledTableRow key={row.id}>
                  ...
                  <StyledTableCell>{row.url ? 'Mevcut' : 'Mevcut Değil'}</StyledTableCell>
