...
export default function KamuoyuDuyurulari() {
    ...
    useEffect(() => {
        setIsLoading(true);
        setHasError(false);
        axios.get(`http://127.0.0.1:8000/api/appname/kamuoyuduyurulari/?page=${currentPage}`)
          .then(response => {
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        if (!editedItem.baslik || !editedItem.kapak_fotografi || !editedItem.tarih || !editedItem.icerik) {
        ...
        axios.put(`http://127.0.0.1:8000/api/appname/kamuoyuduyurulari/${editedItem.id}/`, formData)
          .then(response => {
      ...
      const handleAddNewItem = () => {
        ...
        if (!newItem.baslik || !newItem.kapakFotografi || !newItem.tarih || !newItem.icerik ) {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/kamuoyuduyurulari/', formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(`http://127.0.0.1:8000/api/appname/kamuoyuduyurulari/?page=${currentPage}`);
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleDeleteSelected = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/kamuoyuduyurulari/bulk_soft_delete/', { ids: selectedIds })
          .then(() => axios.get(`http://127.0.0.1:8000/api/appname/kamuoyuduyurulari/`))
          .then(response => {
            ...
            if (updatedPage !== currentPage) {
              return axios.get(`http://127.0.0.1:8000/api/appname/kamuoyuduyurulari/?page=${updatedPage}`);
            } else {
              return response;
            }
          })
          ...
          <Table>
            <TableHead>
              <StyledTableRow>
                ...
                <StyledTableCell>İçerik</StyledTableCell>
            ...
            <TableBody>
              {data.map(row => (
                <StyledTableRow key={row.id}>
                  ...
                  <StyledTableCell>{row.tarih}</StyledTableCell>
                  <StyledTableCell>{row.icerik ? 'Mevcut' : 'Mevcut Değil'}</StyledTableCell>
        ...
        <DialogContent>
            ...
            <LocalizationProvider dateAdapter={AdapterDateFns} locale={trLocale}>
                <DatePicker
                    ...
                    onChange={(newValue) => {
                        // Yerel tarih formatını kullanarak tarihi kaydedin
      ...
      <DialogContent>
       ...
       <div style={{ textAlign: 'center', marginBottom: '20px', border: '2px dashed grey', padding: '10px', position: 'relative' }}> {/* position: relative eklendi */}
        ...
        <TextField
            ...
            fullWidth
            margin="normal"
            style={{ fontSize: '1.2rem' }} // Yazı boyutunu artırın
        />
        <FormControlLabel
