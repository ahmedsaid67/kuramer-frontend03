...
export default function TemelKonular() {
    ...
    useEffect(() => {
        setIsLoading(true);
        setHasError(false);
        axios.get(`http://127.0.0.1:8000/api/appname/temelkonular/?page=${currentPage}`)
          .then(response => {
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        axios.put(`http://127.0.0.1:8000/api/appname/temelkonular/${editedItem.id}/`, formData)
          .then(response => {
      ...
      const handleAddNewItem = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/temelkonular/', formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(`http://127.0.0.1:8000/api/appname/temelkonular/?page=${currentPage}`);
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleDeleteSelected = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/temelkonular/bulk_soft_delete/', { ids: selectedIds })
          .then(() => axios.get(`http://127.0.0.1:8000/api/appname/temelkonular/`))
          .then(response => {
            ...
            if (updatedPage !== currentPage) {
              return axios.get(`http://127.0.0.1:8000/api/appname/temelkonular/?page=${updatedPage}`);
            } else {
              return response;
            }
          })
