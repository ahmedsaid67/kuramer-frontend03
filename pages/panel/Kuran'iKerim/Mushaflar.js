...
export default function Brosurler() {
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:8000/api/appname/mushafkategori-list/')
          .then(response => {
            setMushafKategoriler(response.data);
          })
          .catch(error => {
            console.error('Mushaf kategorilerini alma hatası:', error);
          });
    }, []);
    ...
    useEffect(() => {
        setIsLoading(true);
        setHasError(false);
        axios.get(`http://127.0.0.1:8000/api/appname/mushaflar/?page=${currentPage}`)
          .then(response => {
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem,kategoriId) => {
        ...
        axios.put(`http://127.0.0.1:8000/api/appname/mushaflar/${editedItem.id}/`, formData)
          .then(response => {
      ...
      const handleAddNewItem = () => {
        ...
        if (!newItem.baslik || !newItem.kapakFotografi || !newItem.pdfDosya || !newItem.mushaf_kategori_id) {
        ...
        formData.append("mushaf_kategori_id", newItem["mushaf_kategori_id"]);

        axios.post('http://127.0.0.1:8000/api/appname/mushaflar/', formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(`http://127.0.0.1:8000/api/appname/mushaflar/?page=${currentPage}`);
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleDeleteSelected = () => {
      ...
      axios.post('http://127.0.0.1:8000/api/appname/mushaflar/bulk_soft_delete/', { ids: selectedIds })
        .then(() => axios.get(`http://127.0.0.1:8000/api/appname/mushaflar/`))
        .then(response => {
          ...
          if (updatedPage !== currentPage) {
            return axios.get(`http://127.0.0.1:8000/api/appname/mushaflar/?page=${updatedPage}`);
          } else {
            return response;
          }
        })
    ...
    return(
          ...
          <Table>
            <TableHead>
              <StyledTableRow>
                ...
                <StyledTableCell>Müshaf Kategori Adı</StyledTableCell>
            ...
            <TableBody>
              {data.map(row => (
                <StyledTableRow key={row.id}>
                  ...
                  <StyledTableCell>{row.mushaf_kategori.baslik}</StyledTableCell>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
        ...
        <DialogContent>
            ...
            <FormControl fullWidth>
                <InputLabel style={{ marginBottom: '8px' }}>Kategori</InputLabel>
                <Select
      ...
      <DialogContent>
        ...
        <FormControl fullWidth>
            <InputLabel>Müshaf Kategori</InputLabel>
            <Select
                value={selectedKategori}
                onChange={(e) => setSelectedKategori(e.target.value)}
            >
