...
export default function Brosurler() {
    ...
    useEffect(() => {
        setIsLoading(true);
        setHasError(false);
        axios.get(`http://127.0.0.1:8000/api/appname/brosurler/?page=${currentPage}`)
          .then(response => {
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        axios.put(`http://127.0.0.1:8000/api/appname/brosurler/${editedItem.id}/`, formData)
          .then(response => {
      ...
      const handleAddNewItem = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/brosurler/', formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(`http://127.0.0.1:8000/api/appname/brosurler/?page=${currentPage}`);
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
