...
export default function Bultenler() {
    ...
    useEffect(() => {
        setIsLoading(true);
        setHasError(false);
        axios.get(`http://127.0.0.1:8000/api/appname/bultenler/?page=${currentPage}`)
          .then(response => {
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        axios.put(`http://127.0.0.1:8000/api/appname/bultenler/${editedItem.id}/`, formData)
          .then(response => {
      ...
      const handleAddNewItem = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/bultenler/', formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(`http://127.0.0.1:8000/api/appname/bultenler/?page=${currentPage}`);
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleDeleteSelected = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/appname/bultenler/bulk_soft_delete/', { ids: selectedIds })
          .then(() => axios.get(`http://127.0.0.1:8000/api/appname/bultenler/`))
          .then(response => {
            ...
            if (updatedPage !== currentPage) {
              return axios.get(`http://127.0.0.1:8000/api/appname/bultenler/?page=${updatedPage}`);
            } else {
              return response;
            }
          })
